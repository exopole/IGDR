...
doc = rbind(test1,test2,test3,test4,test5,test6,test7,test8,test9,test10)
output = cbind("mRNA"=doc$mRNA, "ORF"=doc$ORF, "Fickett"=doc$Fickett, "Hexamer" = doc$Hexamer,"Prob"=doc$Prob, "Label"=doc$Label)
write.table(output,file="result",quote=F,sep="\t",row.names=ID[dall])
result = read.table(file="result",header=T,sep="\t")
result
...
xval = moyC2[coord]
xval_moy = mean(xval)
x = paste("cutoff de Accuracy = ", xval_moy)
x
############################################
...
x = paste("cutoff correspondant a l'intersection = ", coordPSx)
x
