...
while(<CPAT>){
	...
	if ($count==0){
		...
		print OUT "$fields[0]\t$fields[1]\t$fields[2]\t$fields[3]\t$fields[4]\t1\t$fields[5]\n";
	}
	elsif (exists($hash_noncoding{lc $id})){
		print OUT "$fields[0]\t$fields[1]\t$fields[2]\t$fields[3]\t$fields[4]\t0\t$fields[5]\n";
	}
	else {
		print OUT "$fields[0]\t$fields[1]\t$fields[2]\t$fields[3]\t$fields[4]\trien\t$fields[5]\n";
	}
}
