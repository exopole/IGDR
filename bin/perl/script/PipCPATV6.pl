...
# shuffle option [choice 2]
	...
	# create of n fasta file of shuffle sequences
   	parse_fasta::fasta_shuffle(-seqA => $PredictC, -seqB => $PredictNC, -seqAprime => $CodingmRNA, -number => $nfile, -cut => $cut,-verbose => $verbose);
	if ($stat ne ""){
...
# intergenic option [choice 3]
	...
	%h_fasta= parse_fasta::fasta2hash(-fastaDirFile => $fastaDirFile, -verbosity => $verbose);	
	...
	if (!$predict){
		@list_size = Utils::predict_size(-seqfileA => $PredictC, -seqfileB => $PredictNC, -seqfileAprime => $CodingORF, -cuttof => $cut);
		print "predict\n";
	}
	...
	parse_fasta::random_seq(-h_fasta => \%h_fasta, -list_size => \@list_size, -h_coord => \%h_coord, -outfile => "$outfile/intergenic.fa", -verbosity => $verbose);
