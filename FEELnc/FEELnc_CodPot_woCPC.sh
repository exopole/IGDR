...
while getopts ":i:p:o:s:v:d:r:gct" opt; do
	case $opt in
   i) # GTF File 
    	# is not readable
    	if [ ! -r "$OPTARG" ];then
...
# Test parameters
### test Methods
if [ -z "$CPAT" ]&& [ -z "$GENEID" ]&&[ -z "$TXCDSPREDICT" ];then
	...
	let "NUMBER_METHOD = $CPAT + $GENEID + $TXCDSPREDICT"
fi
...
perl ${BIN_DIR}/gtf2fasta.pl -i ${INFILE} -o ${OUTDIR}/${INFILE_FASTA} -s ${SLOP} -v 0
if [ "$VERBOSITY" -gt 0 ];then
...
awk -v CPAT_CUT=$CPAT_CUT -v CPC_CUT=$CPC_CUT -v GENEID_CUT=$GENEID_CUT -v TXCDSPREDICT_CUT=$TXCDSPREDICT_CUT -v CPAT=$CPAT -v GENEID=$GENEID -v TXCDSPREDICT=$TXCDSPREDICT '{
	if (CPAT  && GENEID  && TXCDSPREDICT ){
		for (j=13;j<=NF;j++){
	...
	}else if (CPAT  && GENEID  && TXCDSPREDICT=="0"){
		for (j=13;j<=NF;j++){
			bool=0;if ($j == "cpat_score"){   		var=$(j+1);gsub(/\"|;/,"",var);   if (var < CPAT_CUT)          {bool=1}; cpat="cpat_NCstatus \""bool"\";"} 
			bool=0;if ($j == "geneid_score"){		var=$(j+1);gsub(/\"|;/,"",var);   if (var < GENEID_CUT)        {bool=1}; geneid="geneid_NCstatus \""bool"\";"} 
		}
	}else if (CPAT  && TXCDSPREDICT  && GENEID=="0"){
		for (j=13;j<=NF;j++){
			bool=0;if ($j == "cpat_score"){   		var=$(j+1);gsub(/\"|;/,"",var);   if (var < CPAT_CUT)          {bool=1}; cpat="cpat_NCstatus \""bool"\";"} 
			bool=0;if ($j == "txcdspredict_score"){		var=$(j+1);gsub(/\"|;/,"",var);   if (var < TXCDSPREDICT_CUT)        {bool=1}; txcdspredict="txcdspredict_NCstatus \""bool"\";"} 
		}
	}else if (GENEID  && TXCDSPREDICT  && CPAT=="0"){
		for (j=13;j<=NF;j++){
			bool=0;if ($j == "geneid_score"){		var=$(j+1);gsub(/\"|;/,"",var);   if (var < GENEID_CUT)        {bool=1}; geneid="geneid_NCstatus \""bool"\";"} 
			bool=0;if ($j == "txcdspredict_score"){ var=$(j+1);gsub(/\"|;/,"",var);   if (var < TXCDSPREDICT_CUT) {bool=1}; txcdspredict="txcdspredict_NCstatus \""bool"\";"} 		
		}	
	}else if (CPAT && TXCDSPREDICT=="0" && GENEID=="0"){
		for (j=13;j<=NF;j++){
			bool=0;if ($j == "cpat_score"){   		var=$(j+1);gsub(/\"|;/,"",var);   if (var < CPAT_CUT)          {bool=1}; cpat="cpat_NCstatus \""bool"\";"} 
		}
	}else if (CPAT=="0" && GENEID  && TXCDSPREDICT=="0"){
		for (j=13;j<=NF;j++){
			bool=0;if ($j == "geneid_score"){		var=$(j+1);gsub(/\"|;/,"",var);   if (var < GENEID_CUT)        {bool=1}; geneid="geneid_NCstatus \""bool"\";"} 
		}
	}else if (CPAT=="0" && GENEID=="0" && TXCDSPREDICT){
		for (j=13;j<=NF;j++){
			bool=0;if ($j == "txcdspredict_score"){ var=$(j+1);gsub(/\"|;/,"",var);   if (var < TXCDSPREDICT_CUT) {bool=1}; txcdspredict="txcdspredict_NCstatus \""bool"\";"} 		
		}
	}
	# print
	print $0,cpat,geneid,txcdspredict;
}' $OUTFILE >  .tmp
mv .tmp $OUTFILE
...
if [ "$DIAGVENN" ];then
	...
	if [ "$NUMBER_METHOD" != "2" ] && [ "$NUMBER_METHOD" != "3" ];then
		...
		awk -v CPAT=$CPAT -v GENEID=$GENEID -v TXCDSPREDICT=$TXCDSPREDICT '{ 
	...
	if (CPAT  && GENEID  && TXCDSPREDICT ){
		...
		print "#Ids	GeneId"length(ar_geneid_pos)"	TxCdsPredict_"length(ar_txcdspredict_pos);
		for (i in id){
			print i"	"ar_geneid[i]"	"ar_txcdspredict[i];
		}
	}
}' $OUTFILE | tr -d "\";" > ${OUTDIR}/${DIAGVENN}/${NUMBER_METHOD}_Way_VennDiagram_${OUTFILE_PREF}.txt

	# Create Venne png
	CreateVenn.sh ${OUTDIR}/${DIAGVENN}/${NUMBER_METHOD}_Way_VennDiagram_${OUTFILE_PREF}.txt
